...
export const useUserStore = defineStore({
  ...
  actions: {
    ...
    async getUserlist() {
      await api
        .get<DataResponse<Userinfo[]>>("v1/setting/getuser")
        .then((response) => {
    ...
    async getComboBranch() {
      await api
        .get<DataResponse<ComboBranch[]>>("v1/gb/combo_branch/branch")
        .then((response) => {
    ...
    async getComboProfile() {
      await api
        .get<DataResponse<Combo[]>>("v1/gb/combo_sys/profile")
        .then((response) => {
    ...
    async pbExistEmail(Checking: Is_existing) {
      await api
        .post<DataResponse<Userinfo[]>>("v1/pb/user_email", Checking)
        .then(async (response) => {
          if (response.data.success) {
    ...
    async getExistEmail(Checking: Is_existing) {
      await api
        .post<DataResponse<Userinfo[]>>("v1/gb/ch_user_email", Checking)
        .then(async (response) => {
          if (response.data.success) {
            this.userinfo = response.data.data;
          }
    ...
    async create_user(dataForm: Userinfo) {
      return await api
        .post<DataResponse<Userinfo>>(
          baseUrl + "v1/setting/create_user",
          dataForm
        )
        .then(async (response) => {
          if (response.data.success) {

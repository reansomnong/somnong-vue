...
// Stores
import { useCustomerStore } from "../../../stores/pos/Customer";
...
const dataForm = reactive<Customer>({} as Customer);
...
onMounted(async () => {
  resetForm();
  await Customers.ItemsList();
  await Customers.ComboGender();
...
const list_gender = computed(() => {
  return Customers.getGender;
});

const items = computed(() => {
  return Customers.getItemsById;
});

const list_items = computed(() => {
  return Customers.getItemsDetail;
});
...
async function loaddata(id: string) {
  resetForm();
  await Customers.ItemsById(id);

  dataForm.status = "u";
  dataForm.cus_code = items.value[0]["cus_code"] as string;

  dataForm.cus_name = items.value[0]["cus_name"] as string;
...
async function create() {
  ...
  if ($v.value.$invalid) {
    ...
    var response = await Customers.create_customer(dataForm);
    if (response) {
                    ...
                    <Dialog.Description class="grid grid-cols-12 gap-4 gap-y-3">
                      <div class="col-span-12 sm:col-span-6">
                        ...
                        <FormInput
                          ...
                          :error="$v.cus_name.$error"
                          :class="{
                            'border-danger': $v.cus_name.$error,
                          }"
                        />
                      </div>

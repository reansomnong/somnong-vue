...
import { useAuthTransferStore } from "../../../stores/pos/AuthStockTransfer";
...
const StockTransfer = useAuthTransferStore();
...
onMounted(async () => {
  await StockTransfer.una_transfer();
});

const una_transfer = computed(() => {
  return StockTransfer.getItems;
});

const una_purchase_view = computed(() => {
  return StockTransfer.getItemView;
});

const una_po_details = computed(() => {
  return StockTransfer.getItemsDetail;
});

async function una_po_view(id: string) {
  await StockTransfer.view_una_transfer(id);
  await StockTransfer.view_una_transfer_details(id);

  dataForm.status = "auth_transfer";
  dataForm.tran_code = una_purchase_view.value[0].tran_code;
  dataForm.stock_name = una_purchase_view.value[0].stock_name;
  dataForm.sto_code_from = una_purchase_view.value[0].sto_code_from;
  dataForm.sto_code_to = una_purchase_view.value[0].sto_code_to;
  dataForm.inputter = una_purchase_view.value[0].inputter;
...
async function auth_transactions() {
  ...
  if ($v.value.$invalid) {
    ...
    var response = await StockTransfer.auth_transfer(dataForm);
    if (response) {
      const failedEl = document
...
async function reject_transfer() {
  ...
  if ($v.value.$invalid) {
    ...
    dataForm.status = "reject_auth_transfer";
    var response = await StockTransfer.reject_transfer(dataForm);
    if (response) {
      const failedEl = document
      ...
      <Dialog.Panel>
        <div class="p-5 text-center">
          ...
          <div class="mt-2 text-slate-500">
            Authorize Stock transfer {{ dataForm.tran_code }}
          </div>
        </div>
    ...
    >
      <Dialog.Panel>
        <div class="p-5 text-center">
          ...
          <div class="mt-5 text-3xl">Are you sure?</div>
          <div class="mt-2 text-slate-500">
            Reject Stock transfer {{ dataForm.tran_code }}
          </div>
        </div>
        <div class="px-5 pb-8 text-center">
          <Button

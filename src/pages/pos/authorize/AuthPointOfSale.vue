...
import { useAuthPOSStore } from "../../../stores/pos/AuthPOS";
...
onMounted(async () => {
  await POS.una_point_of_sale();
});

const una_point_of_sale = computed(() => {
  return POS.getItems;
});

const una_pos_view = computed(() => {
  return POS.getItemView;
});

const una_pos_details = computed(() => {
  return POS.getItemsDetail;
});

async function una_po_view(id: string) {
  console.log(id);
  await POS.view_una_pos(id);
  await POS.view_una_po_details(id);

  if (una_pos_view.value.length > 0) {
    dataForm.status = "auth_pos";
    dataForm.pos_code = una_pos_view.value[0].pos_code;
    dataForm.cus_code = una_pos_view.value[0].cus_code;
    dataForm.table_num = una_pos_view.value[0].table_num;
    dataForm.people_num = una_pos_view.value[0].people_num;
    dataForm.subtotal = una_pos_view.value[0].subtotal;
    dataForm.disamount = una_pos_view.value[0].disamount;
    dataForm.remark = una_pos_view.value[0].remark;
    dataForm.total_amount = una_pos_view.value[0].total_amount;
...
async function auth_poin_of_sale() {
  $v.value.$touch();
  if ($v.value.$invalid) {
    ...
    var response = await POS.auth_point_of_sale(dataForm);
    if (response) {
      const failedEl = document
...
async function reject_point_of_sale() {
  $v.value.$touch();
  if ($v.value.$invalid) {
    const failedEl = document
      .querySelectorAll("#failed-notification-content")[0]
    ...
    dataForm.status = "reject_auth_pos";
    var response = await POS.reject_point_of_sale(dataForm);
    if (response) {
      const failedEl = document
        .querySelectorAll("#success-notification-content")[0]
        .cloneNode(true) as HTMLElement;
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
    ...
    <div
      ...
      v-if="una_point_of_sale.length > 0"
    >
      <Table class="border-spacing-y-[10px] border-separate -mt-2">
        <Table.Thead>
          <Table.Tr>
            <Table.Th class="border-b-0 whitespace-nowrap"> Invoice </Table.Th>
            <Table.Th class="border-b-0 whitespace-nowrap"> Customer </Table.Th>
        ...
        <Table.Tbody>
          <Table.Tr
            v-for="list in una_point_of_sale"
            :key="list.pos_code"
            class="intro-x"
          >
            <Table.Td
                  ...
                  <Dialog.Panel>
                    ...
                    <Dialog.Description class="grid grid-cols-12 gap-4 gap-y-3">
                      <div class="col-span-12 sm:col-span-6">
                        ...
                        <FormInput
                          :value="dataForm.pos_code"
                          type="text"
                          placeholder="invoice"
                          v-model="dataForm.pos_code"
                          :disabled="true"
                        />
                      </div>
                      <div class="col-span-12 sm:col-span-6">
                        <FormLabel htmlFor="modal-form-3"> Customer </FormLabel>
                        <FormInput
                          type="text"
                          :value="dataForm.cus_name"
                          placeholder="Customer name"
                          v-model="dataForm.cus_name"
                          :disabled="true"
                        />
                      </div>
                      ...
                      <div class="col-span-12 sm:col-span-6">
                        ...
                        <FormInput
                          type="text"
                          :value="dataForm.disamount"
                          v-model="dataForm.disamount"
                          :disabled="true"
                        />
                      </div>
      ...
      <Dialog.Panel>
        <div class="p-5 text-center">
          ...
          <div class="mt-2 text-slate-500">
            Authorize POS invoice {{ dataForm.pos_code }}
          </div>
        </div>
    ...
    >
      <Dialog.Panel>
        <div class="p-5 text-center">
          ...
          <div class="mt-5 text-3xl">Are you sure?</div>
          <div class="mt-2 text-slate-500">
            Reject purchase order {{ dataForm.pos_code }}
          </div>
        </div>
        <div class="px-5 pb-8 text-center">
          <Button
